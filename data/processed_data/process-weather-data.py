			...
			for yearFolder in os.listdir(allYears):
				...
				for file in os.listdir(f'{allYears}/{yearFolder}'):
					try:
						...
						startIndex = 3
						month, day = content[startIndex][1], content[startIndex][2]
						for rowNum in range(startIndex, len(content[startIndex:])): #skip the first 3 rows
							try:
								row = content[rowNum]
								if row[1] != month or row[2] != day: #day/month changes = new row; accounts for any missed hourly data
									# date = f'{row[0]}-{row[1]}-{row[2]-1}'
									date = f'{row[0]}-{month}-{day}'
									...
									#12 fields per day
									for i in content[startIndex:rowNum+1]:
										avgDBT += i[4]
										avgDPT += i[5]
										avgPress += i[6]
										avgSkyCov += i[7]
									...
									numEntries = rowNum+1-startIndex
